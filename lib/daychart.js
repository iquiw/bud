...
export default class DayChart {
   ...
   * @param {option} altChart - Alternate chart data.
   */
  draw(startDay, endDay, dueDay, burndown, altChart) {
    let days = [];
    let {
      downs, ups,
    ...
    for (let i = 0, estimate = downs[0], optimate = null, ideal = downs[0],
      ...
      if (altChart) {
        let aday = null;
        for (; altIndex < altChart.data.length; altIndex++) {
          let x = altChart.data[altIndex];
          aday = Day.fromString(x[0]);
          if (aday.isAfter(day)) {
            break;
          }
        }
        if (altIndex == altChart.data.length) {
          if (aday && aday.isSame(day)) {
            altData.push(altChart.data[altIndex - 1][1]);
          }
        } else if (altIndex > 0) {
            altIndex--;
            altData.push(altChart.data[altIndex][1]);
        } else if (altIndex == 0) {
          altData.push(0);
        }
      }
    ...
    let beforeBuildTicks = null;
    if (altChart) {
        datasets.push(DayChart.lineDataset(altChart.legend, this.colorEstimate, altData));
    } else {
      ...
      options: {
        ...
        scales: {
          ...
          line: {
            ...
            stacked: false,
            title: {
              display: altChart ? altChart.text : false,
              text: altChart ? altChart.text : null,
            },
            min: altChart ? altChart.min : 0,
            max: altChart ? altChart.max : null,
            ticks: {
              stepSize: altChart ? altChart.stepSize : 1,
              beforeBuildTicks,
            },
            position: 'right',
          }
